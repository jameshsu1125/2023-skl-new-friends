...
const RegularButton = memo(({ children, invert, onClick, prev, hideIcon }) => {
	const className = useMemo(() => {
		const classes = [
			'RegularButton flex pointer-events-auto flex-row border-tertiary items-center justify-center rounded-l-full rounded-r-full border-2 px-5 py-2 text-2xl tracking-widest xl:text-base',
		];

...
const RegularButton = memo(({ children, invert, onClick, prev, hideIcon, color = 'pink' }) => {
	const className = useMemo(() => {
		const classes = [
			'RegularButton flex pointer-events-auto flex-row items-center justify-center rounded-l-full rounded-r-full border-2 px-5 py-2 text-2xl tracking-widest xl:text-base',
		];
		if (invert) classes.push('RegularButton-tertiary');
		else classes.push('RegularButton-white');
		if (prev) classes.push('RegularButton-prev-hover');
		if (hideIcon) classes.push('RegularButton-hideIcon');

		if (color === 'orange') classes.push('border-orange');
		else classes.push('border-tertiary');

		return classes.join(' ');
	}, [invert, prev, hideIcon, color]);

	const iconFill = useMemo(() => {
		if (color === 'orange') return '#FED27B';
		else if (invert) return '#fff';
		else return '#FFA79D';
	}, [invert, color]);
